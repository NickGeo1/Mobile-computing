...
@Composable
fun ProfileActivity(nav: NavHostController, username: String) {
    Surface(modifier = Modifier.fillMaxSize().systemBarsPadding(), color = bgyellow) {
        ...
        Box {
            TopAppBar(
                ...
                defTopbarTextandIconbutton(
                    onclick = { nav.navigate("main/${username}") },
        ...
        Column(
            ...
            defButton(onclick = { UserInitialisaton().updateUser(username, txtusername.value, nav)
                                }, text = "Change")
        }
    }
}
...
fun DefaultPreview2() {
    ...
    ProfileActivity(nav = appState.navController, "Test")
